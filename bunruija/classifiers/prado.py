...
#         for i in range(0, self.n_features, 64):
#             hash_ = mmh3.hash128(word)
#             print(word, hash_)
...
class ProjectedAttention(torch.nn.Module):
    def __init__(self, kernel_sizes, dim_hid):
        super().__init__()
    ...
    def __call__(self, x_value_in, x_attn_in):
        ...
        for conv_value, conv_attn in zip(self.convs_value, self.convs_attn):
...
class PRADO(NeuralBaseClassifier):
    def __init__(self, **kwargs):
        ...
        self.attention = ProjectedAttention(self.kernel_sizes, self.dim_hid)
